...
pub fn load_config() -> Result<DocsConfig, String> {
    config_io::load_json_config(PRODUCT_ID, CONFIG_FILENAME, DocsConfig::default())
}
...
mod tests {
    ...
    fn config_round_trips_through_json() {
        ...
        let parsed: DocsConfig = serde_json::from_str(&json).expect("deserialize");
    ...
    fn config_deserialize_with_missing_fields_uses_defaults() {
        ...
        let config: DocsConfig = serde_json::from_str(json).expect("partial config");
    ...
    fn load_config_returns_default_when_missing() {
        ...
        assert_eq!(config.editor.font_size, 16);
        assert_eq!(config.save.default_format, OfficeFileFormat::Docx);
    }
