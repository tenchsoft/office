use super::state::{c_accent, c_ruler_bg, c_separator, c_text_dim, DocsState, RulerDragTarget};
...
pub fn paint_ruler(p: &mut Painter<'_>, cache: &mut TextCache, rect: Rect, state: &DocsState) {
...
fn extract_indents(block: &tench_document_core::BlockNode) -> (f32, f32, f32) {
    extract_indents_for_automation(block)
}

/// Public wrapper used by `docs_automation_nodes` to compute ruler handle positions.
pub fn extract_indents_for_automation(block: &tench_document_core::BlockNode) -> (f32, f32, f32) {
    match block {
...
pub fn ruler_hit_test(x: f64, rect: Rect, state: &DocsState) -> Option<(RulerDragTarget, f64)> {
    let scale = state.zoom / 100.0;
    let doc = state.current_document();
    let setup = &doc.page_setup;
    ...
    if let Some(block) = doc.content.get(state.cursor().block_idx) {
        ...
        if (x - indent_left_x).abs() < hit_radius {
            return Some((RulerDragTarget::IndentLeft, indent_left as f64));
        }
        if (x - indent_right_x).abs() < hit_radius {
...
pub fn ruler_drag_to_margin(x: f64, rect: Rect, state: &DocsState) -> f32 {
    let scale = state.zoom / 100.0;
    let doc = state.current_document();
    let setup = &doc.page_setup;
    let (page_w_raw, _) = setup.page_size_px();
...
pub fn ruler_drag_to_indent(x: f64, rect: Rect, state: &DocsState) -> f32 {
    let scale = state.zoom / 100.0;
    let doc = state.current_document();
    let setup = &doc.page_setup;
    let (page_w_raw, _) = setup.page_size_px();
