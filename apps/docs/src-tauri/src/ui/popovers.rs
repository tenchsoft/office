...
pub(super) fn paint_tab_bar(
    ...
    for (idx, tab) in state.open_tabs.iter().enumerate() {
        ...
        // Tab title (truncate if needed, using char boundary)
        let title = if tab.title.chars().count() > 18 {
            let truncated: String = tab.title.chars().take(15).collect();
            format!("{truncated}...")
        } else if tab.dirty {
            format!("{} *", tab.title)
        } else {
...
/// Computed by walking TOOLBAR_LAYOUT to find the InsertTable item.
pub(super) fn table_grid_x() -> f64 {
    let mut left = TOOLBAR_LEFT_PAD;
    let mut prev_group = 0;
    let mut after_dropdowns = false;

    for item in TOOLBAR_LAYOUT.iter() {
        if item.group != prev_group {
            left += toolbar_actions::SEPARATOR_W;
            // Insert dropdown groups between group 0 and group 4
            if prev_group == 0 && item.group >= 4 && !after_dropdowns {
                left += FONT_SIZE_SELECT_W
                    + BTN_GAP
                    + toolbar_actions::SEPARATOR_W
                    + FONT_FAMILY_SELECT_W
                    + BTN_GAP
                    + toolbar_actions::SEPARATOR_W
                    + PARAGRAPH_SELECT_W
                    + BTN_GAP;
                after_dropdowns = true;
            }
            prev_group = item.group;
        }

        if matches!(item.action, toolbar_actions::ToolbarAction::InsertTable) {
            return left;
        }
...
/// Computed by walking TOOLBAR_LAYOUT to find the TextColor item.
pub(super) fn color_picker_x() -> f64 {
    ...
    for item in TOOLBAR_LAYOUT.iter() {
        ...
        if matches!(item.action, toolbar_actions::ToolbarAction::TextColor) {
            return left;
        }
        left += item.width + BTN_GAP;
    }
