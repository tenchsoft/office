...
/// Render a sequence of inline nodes with formatting.
...
pub(crate) fn paint_inline_nodes(
    ...
    // Track last marks for trailing flush (Phase 1.3)
    ...
    for node in nodes {
        match node {
            ...
            InlineNode::Link {
                ...
                for word in words {
                    if line_buf.len() + word.len() + 1 > chars_per_line && !line_buf.is_empty() {
                        ...
                        p.draw_line(
                            Point::new(line_x, line_y + 2.0 * scale),
                            Point::new(line_x + text_w, line_y + 2.0 * scale),
                            link_color,
                            1.0,
                        );
                        flat_text.push_str(&line_buf);
            ...
            InlineNode::HardBreak => {
                if !line_buf.is_empty() {
                    let line_x =
                        compute_aligned_x(left, content_w, &line_buf, base_font_size, alignment);
                    p.draw_text_cached(
                        cache,
                        &line_buf,
                        line_x,
    ...
    // Flush remaining text with preserved marks from last text node
    if !line_buf.is_empty() {
...
// UI painting functions need many layout parameters
#[allow(clippy::too_many_arguments)]
/// Render inline nodes with alignment support for paragraph blocks.
pub(crate) fn paint_inline_nodes_with_alignment(
    p: &mut Painter<'_>,
    cache: &mut TextCache,
    nodes: &[InlineNode],
    left: f64,
