...
pub(crate) fn paint_page_header(
    ...
    // Draw header content if configured
    if let Some(header_text) = hf.header_for_page(page, pages, title) {
        p.draw_text_cached(
            cache,
            &header_text,
            left,
...
pub(crate) fn paint_page_footer(
    ...
    // Draw footer content if configured, otherwise page number
    if let Some(footer_text) = hf.footer_for_page(page, pages, title) {
        p.draw_text_cached(
            cache,
            &footer_text,
            left,
...
pub(crate) fn estimate_block_height(
    ...
    match block {
        ...
        BlockNode::Footnote { content, .. } => {
            ...
            lines as f64 * (16.0 * scale) + block_spacing
        }
...
pub(crate) fn paint_track_change_decorations(
    ...
    for change in &state.tracked_changes {
        ...
        let (decoration_color, is_underline) = match change.change_type {
            ChangeType::Insert => (Color::rgba8(0x66, 0xBB, 0x6A, 200), true),
            ChangeType::Delete => (Color::rgba8(0xEF, 0x53, 0x50, 200), false),
        };
