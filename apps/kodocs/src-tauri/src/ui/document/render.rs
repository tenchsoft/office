...
/// that should be rendered upright in vertical writing mode.
...
pub(super) fn paint_block(
    ...
    match block {
        ...
        BlockNode::Footnote { number, content } => paint_footnote_block(
            p, cache, *number, content, left, y, content_w, scale, bottom,
        ),
    }
}
...
#[allow(clippy::too_many_arguments)]
pub(super) fn paint_footnote_block(
    p: &mut Painter<'_>,
    cache: &mut TextCache,
    number: u32,
    content: &[InlineNode],
    ...
    let mut draw_y = y;
    for line in wrap_text(&display, content_w, font_size, scale) {
        if draw_y + line_h > bottom {
            break;
        }
        p.draw_text_cached(
            cache,
...
pub(super) fn paint_image_block(
    ...
    // Image selection handles (small squares at corners)
